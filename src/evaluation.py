...
def generate_report(classifier, metrics: Dict, df: pd.DataFrame, predictions: list) -> str:
    """
    ...
    for label in classifier.difficulty_levels:
        ...
        report_lines.append(f"{label.upper():10} {count:3d} samples ({percentage:5.1f}%)")
    ...
    header = "        " + " ".join(f"{label:>8}" for label in classifier.difficulty_levels)
    report_lines.append(header)
    
    for i, label in enumerate(classifier.difficulty_levels):
        row = f"{label:>8}" + " ".join(f"{cm[i, j]:>8}" for j in range(len(classifier.difficulty_levels)))
